...
key_report_response_frame_t* command_logic_key_report_shutter(void) {
    ESP_LOGI(TAG, "%s: Reporting shutter button press", __FUNCTION__);

    if (connect_logic_get_state() != PROTOCOL_CONNECTED) {
        ESP_LOGE(TAG, "Camera protocol is not connected. Current state: %d",
    ...
    CommandResult result = send_command(
        0x00, 0x11, CMD_RESPONSE_OR_NOT, &command_frame, seq, 5000);
