...
async function refresh(){try{let s=await (await fetch('/status',{cache:'no-store'})).json();
	...
	$('pairingHelp').innerHTML='Placez la GoPro en mode Photo, puis ouvrez Connexions › Connecter un appareil et lancez la recherche. Bouton BOOT : appui court = photo, 3 secondes = recherche/appairage, 10 secondes = oubli.';
