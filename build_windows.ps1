...
Remove-Item "build-windows-x64" -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item "dist-windows\Hackman3D LayerShot x64" -Recurse -Force -ErrorAction SilentlyContinue
$env:LAYERSHOT_BUILD_ARCH = "x64"
& ".venv-windows\Scripts\python.exe" -m PyInstaller --noconfirm --clean `
    --workpath "build-windows-x64" --distpath "dist-windows" "Hackman3D LayerShot.spec"
...
Remove-Item "build-windows-x86" -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item "dist-windows\Hackman3D LayerShot x86" -Recurse -Force -ErrorAction SilentlyContinue
$env:LAYERSHOT_BUILD_ARCH = "x86"
& ".venv-windows-x86\Scripts\python.exe" -m PyInstaller --noconfirm --clean `
    --workpath "build-windows-x86" --distpath "dist-windows" "Hackman3D LayerShot.spec"
...
Remove-Item "releases\Hackman3D-LayerShot-Windows-Setup-1.2.9.exe" -Force -ErrorAction SilentlyContinue
& $InnoCompiler "installer\windows\Hackman3D-LayerShot.iss"
if ($LASTEXITCODE -ne 0) { throw "Windows installer creation failed." }
Write-Host "Created releases\Hackman3D-LayerShot-Windows-Setup-1.2.9.exe"
